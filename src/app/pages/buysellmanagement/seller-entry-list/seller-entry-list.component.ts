...
@Component({
  selector: 'app-seller-entry-list',
  templateUrl: './seller-entry-list.component.html',
  styleUrls: ['./seller-entry-list.component.scss']
})
export class SellerEntryListComponent implements OnInit {
  displayedColumns: string[] = ['EntryDate','BuyerName', 'Total','Discount','Cashed','Remain','Status'];
  definedColumns:any[]= [
  ...
  ngOnInit() {
    this.breadCrumbItems = [{ label: 'ক্রয়/বিক্রয়' }, { label: 'সকল বিক্রয়  তথ্য ', active: true }];
  ...
  gotoEditPage(id){   
    this.router.navigate(['/buySellManagement/sellerEntry'], { queryParams: {id: btoa(id)}});
  }
  ...
  Editdelete(data:any,type:number){    
    ...
    Swal.fire({
      ...
      if (result.value) {   
       ...
       this.apiservice.getData(this.apiservice.deleteBuyerInfo+"?status=-1&id="+buyerId).subscribe(data => {
  ...
  getTableData() {
    ...
    this.apiservice.getData(this.apiservice.getBuyerInfoManagementList+"?userId="+id+"&status=1")
    .subscribe(dd => {
